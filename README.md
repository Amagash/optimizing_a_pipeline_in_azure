...
Random and Bayesian methods are more efficient than grid search. For this project, the `RandomParameterSampling` was chosen as the parameter sampler because it is more optimized than grid search and we can use an early termination policy which is not the case with the Bayesian method.
